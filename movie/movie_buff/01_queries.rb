...
def harrison_ford
  ...
  Movie.joins(:actors).select('movies.id', :title).where("ord > 1 AND name = 'Harrison Ford'")
  
  # Casting.joins(movies).select(:title).where("ord > ?", 1)
...
def biggest_cast
  ...
  Movie.joins(:castings).select('movies.id', :title).group('movies.id').order('COUNT(actor_id) DESC').limit(3)
